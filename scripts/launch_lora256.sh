#!/bin/bash
# Launch 12 conditions for LoRA r=256 across 8 GPUs.
# 3 models × 4 contamination levels = 12 jobs
#
# Usage:
#   cd ~/final_proj && nohup bash scripts/launch_lora256.sh > outputs/gpu_full_run/launch_lora256.log 2>&1 &
...
echo "$(date): Launching $TOTAL conditions for $FT across 8 GPUs"

# Data should already be prepared from the lora8 run
...
echo ""
echo "$(date): All $TOTAL $FT jobs launched. Waiting..."
...
echo ""
echo "$(date): $FT done. Failed: $FAILED / $TOTAL"
echo "=== SUMMARY ==="
for LOG in "$LOG_DIR"/${FT}_*.log; do
